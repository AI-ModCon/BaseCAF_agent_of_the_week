...
Scientific question answering over a specialized materials corpus requires capabilities that neither a standard RAG pipeline nor a plain LLM call provides reliably. The core difficulty is that answering a real materials question needs *both* executable computation over experimental values *and* mechanistic grounding from the literature &mdash; and each retrieval mode alone misses half the answer:

<p align="center">
  <img src="images/12-MagQuery/Ordinary_RAG.png" width="70%" alt="SQL-only retrieval computes over experimental values but misses mechanism and citations; literature-only RAG explains mechanism but cannot compute over the data. MagQuery fuses both.">
  <br><em>Figure 1: Ordinary RAG is not sufficient. SQL-only / text-to-SQL computes joins, ratios, and maxima but misses mechanisms, caveats, and citations; literature-only RAG explains mechanisms with cited passages but cannot touch the experimental data. MagQuery's SUQL fuses the two.</em>
</p>

MagQuery's architecture is motivated by three requirements that drive each design decision:
...
- **Orchestration matters more than tool access:** A rigorous ablation study (6 conditions &times; 3 shots across MagQuery-Bench, 752 citation-referenced items) isolates the contribution of each component. The key empirical finding: the full orchestration system outperforms simply giving the LLM access to the same tools without structured orchestration by **+0.10 on the public test set and +0.15 on the private test set** &mdash; demonstrating that how the agent is instructed to use its tools is as important as which tools it has.
...
<p align="center">
  ...
  <br><em>Figure 2: MagQuery architecture. A DSPy multi-hop planner fuses structured SQL over the materials database with literature retrieval (BM25 + embeddings) via SUQL in a single loop, then a Python-REPL answer-extraction step returns an exact, citation-grounded answer &mdash; or abstains. Tools are served over MCP/FastAPI; the LLM backend is model-agnostic (LiteLLM); prompts are tuned with DSPy and traced with MLflow. The materials database and literature index together form the MagQuery dataset, with full source provenance.</em>
</p>

The agent exposes both a CLI (`mq ask`) and an **MCP server**, enabling integration into broader agentic workflows. DSPy optimization (`mq train`) produces optimized-prompt artifacts stored separately from code &mdash; no model weights are trained or redistributed. The full stack is deployable via Docker Compose.

We evaluate on **MagQuery-Bench**, a benchmark of 752 citation-referenced Q/A items across `dev` / `test_public` / `test_private` splits, scored by an LLM judge against gold answers and citations. A six-condition ablation isolates each component's contribution (Figure 3): the full system scores **0.402 / 0.503 / 0.525** (dev / public / private) and beats an otherwise-identical agent given the same tools *without* structured orchestration by **+0.10\*\*\* (public)** and **+0.15\*\*\* (private)** &mdash; direct evidence that orchestration, not mere tool access, drives performance. Conditions without database access (`rag_only`, `web_search`, `closed_book`) collapse on database-grounded questions, and `closed_book` fabricates citations by design &mdash; underscoring why grounded retrieval and abstention are essential.

<p align="center">
  <img src="images/12-MagQuery/ablation.png" width="88%" alt="Grouped bar chart of mean score by condition and split on MagQuery-Bench. The full system is highest on every split (0.40/0.50/0.53); sql_only and tools_no_orchestration trail; rag_only, web_search, and closed_book collapse.">
  <br><em>Figure 3: Component ablation on MagQuery-Bench (3-shot, LLM-judged). The full system is best on every split; removing structured orchestration costs +0.10*** (public) / +0.15*** (private), and retrieval-ablated conditions collapse because they cannot reach the structured property values.</em>
</p>
...
### Source Code
- **Repository:** Private &mdash; DOE CODE / OSTI submission pending; available from the authors on request.
- **License:** Apache-2.0.
...
- **MagQuery dataset:** a sibling DOE submission (DOE Data Explorer) &mdash; the BibTeX-indexed literature corpus and magnetic-materials property database that MagQuery retrieves over.
- **Contact:** Sreya Vangara &mdash; svangara@stanford.edu, Stanford University (ORCID: 0000-0002-4762-8773)
- **Contact:** Johannes Voss &mdash; vossj@slac.stanford.edu, SLAC National Accelerator Laboratory (ORCID: 0000-0001-7740-8811)
- **Contact:** Eric Darve (PI) &mdash; darve@stanford.edu, Stanford University (ORCID: 0000-0002-1938-3836)

---

*Last Updated: July 23, 2026*

*Project status: Active development (paper in preparation)*

*Contributed by: Sreya Vangara, Jonathan Thompson (Stanford ICME; ORCID 0009-0001-1367-865X), Eric Darve (PI) &mdash; Stanford University; Johannes Voss &mdash; SLAC National Accelerator Laboratory*
